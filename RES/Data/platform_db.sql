...
--
-- Temporary view structure for view `good_card`
--

DROP TABLE IF EXISTS `good_card`;
/*!50001 DROP VIEW IF EXISTS `good_card`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `good_card` AS SELECT 
 1 AS `item_name`,
 1 AS `desc`,
 1 AS `itemFront`,
 1 AS `price`,
...
CREATE TABLE `item_pictures` (
  ...
  KEY `fk_item_pictures_item_idx` (`item_id`),
  CONSTRAINT `fk_item_pictures_item` FOREIGN KEY (`item_id`) REFERENCES `item` (`item_id`)
...
--
-- Final view structure for view `good_card`
--
...
/*!50001 VIEW `good_card` AS select `i`.`item_name` AS `item_name`,`i`.`desc` AS `desc`,`i`.`thumbnail` AS `itemFront`,`i`.`price` AS `price`,`u`.`userName` AS `username`,`u`.`location` AS `location`,`u`.`thumbnail` AS `userthumbnail` from (`item` `i` left join `user` `u` on((`i`.`seller_id` = `u`.`user_id`))) */;
...
-- Dump completed on 2024-02-15 10:33:26
